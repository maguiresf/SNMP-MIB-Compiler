    ...
    if ( $c eq '\'' ) {    # it can be a cstring or a hstring.
        ...
        if ( $c eq '\'' ) {
            ...
            return $self->assert( MIBERROR, $self->{'filename'},
                $s->{'lineno'}, "Unexpected EOF near \"$val\"" )
                if $c eq '';
            $val .= $c;
    ...
    if ( $c eq ':' ) {    # an assignment item
        ...
        if ( $c ne ':' ) {
            ...
            return $self->assert( MIBERROR, $self->{'filename'},
                ...
                if $c eq '';
            return $self->assert( MIBERROR, $self->{'filename'},
                $s->{'lineno'}, "Syntax error near \"$val\"" )
                unless $c eq '=';
            $val .= $c;
...
sub compile {
    ...
    if ( $self->{'use_dump'} && -e "$outdir/$file$self->{'dumpext'}" ) {
        if ( -M $filename < -M "$outdir/$file$self->{'dumpext'}" ) {
            ...
            if ( defined $fh ) {
                ...
                $v = eval <$fh>;
                if ($v) {
                    map { $self->{'nodes'}{$_} = $$v{'nodes'}{$_} }
                        keys %{ $$v{'nodes'} };
                    map { $self->{'types'}{$_} = $$v{'types'}{$_} }
                        keys %{ $$v{'types'} };
                    ...
                    map { $self->{'traps'}{$_} = $$v{'traps'}{$_} }
                        keys %{ $$v{'traps'} };
                    map { push @{ $self->{'macros'} }, $_ }
                        @{ $$v{'macros'} };
                }
                $fh->close;
            }
    ...
    map { $self->{'nodes'}{$_} = $mib->{'nodes'}{$_} }
        keys %{ $mib->{'nodes'} };
    map { $self->{'types'}{$_} = $mib->{'types'}{$_} }
        keys %{ $mib->{'types'} };
    map { $self->{'traps'}{$_} = $mib->{'traps'}{$_} }
        keys %{ $mib->{'traps'} };
    ...
    while ( $token && $token != $END ) {
        if ( $token == $IMPORTS ) {
            ...
            if ( $token == $OBJECT ) {    # probably an OBJECT IDENTIFIER
                ...
                $self->assert( MIBERROR, $self->{'filename'},
        ...
        if ( defined $self->{'nodes'}{ $$list[0] } ) {
                ...
                == $#{ $self->{'nodes'}{ $$list[0] }{'oid'} } )
            {
                ...
                for ( @{ $self->{'nodes'}{ $$list[0] }{'OID'} } ) {
                    $i++;
                    $eq = 0, last
                        unless ${ $self->{'nodes'}{ $$list[0] }{'OID'} }[$i]
...
sub parse_one {
    ...
    if ( $value eq '-' ) {    # a negative value ?
        ( ( $token, $value ) = $self->get_token() ) || return;
        return $self->assert( MIBERROR, $self->{'filename'},
            $self->{'lineno'}, "\"$value\" must be an integer" )
            unless $token == $NUMBER;
        $value = "-$value";
    }
    my $val = $value;
    ...
    if ( $token && $value eq '(' ) {
        ...
        while ( $value ne '}' ) {
            ...
            if ( $token == $IDENTIFIER ) {
                ...
                return $self->assert( MIBERROR, $self->{'filename'},
                    $self->{'lineno'}, "should be an identifier" );
            }
            ( ( $token, $value ) = $self->get_token() ) || return;
            return $self->assert( MIBERROR, $self->{'filename'},
                $self->{'lineno'},
...
sub parse_type {
    ...
    if ( $token == $IMPLICIT ) {    # implicit types
        ...
        my $type = "INTEGER";
        my $subtype =
            $self->parse_subtype();     # what if parse_subtype failed ???
        my $ref = ref $subtype;
        if ( defined $ref && $ref eq 'HASH' ) {
            $$subtype{'type'} = $type;
            return $subtype;
...
sub parse_textualconvention {
    ...
    if ( $value eq 'STATUS' ) {
        ...
        return $self->assert( MIBERROR, $self->{'filename'},
            $self->{'lineno'}, "Syntax error in TC: 'STATUS' requiered" );
    }
    ...
    if ( $value eq 'SYNTAX' ) {
        ...
        return $self->assert( MIBERROR, $self->{'filename'},
            $self->{'lineno'}, "Syntax error in TC: 'SYNTAX' requiered" );
    }
    $data;
}
...
sub parse_objectidentity {
    ...
    if ( $value eq 'STATUS' ) {
        ...
        return $self->assert( MIBERROR, $self->{'filename'},
            $self->{'lineno'}, "Syntax error. 'STATUS' needed" );
    }
    if ( $value eq 'DESCRIPTION' ) {
        ( ( $token, $value ) = $self->get_token('CSTRING') ) || return;
        $$data{'description'} = $value;
...
sub parse_moduleidentity {
    ...
    if ( $value eq 'LAST-UPDATED' ) {
        ...
        return $self->assert( MIBERROR, $self->{'filename'},
            $self->{'lineno'}, "Syntax error. 'LAST-UPDATED' needed" );
    }
    if ( $value eq 'ORGANIZATION' ) {
        ...
        return $self->assert( MIBERROR, $self->{'filename'},
            $self->{'lineno'}, "Syntax error. 'ORGANIZATION' needed" );
    }
    if ( $value eq 'CONTACT-INFO' ) {
        ...
        return $self->assert( MIBERROR, $self->{'filename'},
            $self->{'lineno'}, "Syntax error. 'CONTACT-INFO' needed" );
    }
    if ( $value eq 'DESCRIPTION' ) {
        ( ( $token, $value ) = $self->get_token('CSTRING') ) || return;
        $$data{'description'} = $value;
        ...
        while ( $value ne 'MODULE' && $token != $ASSIGNMENT ) {
            if ( $value eq 'MANDATORY-GROUPS' ) {
                ...
                if ( $value eq 'WRITE-SYNTAX' ) {
                    my $type = $self->parse_type();
                    $$data{'module'}{$name}{'object'}{$val}{'write-syntax'} =
                        $type;
                    ( ( $token, $value ) = $self->get_token() ) || return;
                }
			    ...
			    accessible-for-notify)$/ox
                    ...
                    {
                        $$data{'module'}{$name}{'object'}{$val}
                            {'min-access'} = $value;
                    } else {
                        return $self->assert( MIBERROR, $self->{'filename'},
                            $self->{'lineno'},
                            "Unknown MIN-ACCESS type ($value)" );
                    }
                    ( ( $token, $value ) = $self->get_token() ) || return;
                }
                if ( $value eq 'DESCRIPTION' ) {
                    ( ( $token, $value ) = $self->get_token('CSTRING') )
                        || return;
                    $$data{'module'}{$name}{'object'}{$val}{'description'} =
                        $value;
                    ( ( $token, $value ) = $self->get_token() ) || return;
                }
                $self->unget_token();
...
sub parse_objectgroup {
    ...
    if ( $value eq 'OBJECTS' ) {
        ...
        return $self->assert( MIBERROR, $self->{'filename'},
            $self->{'lineno'}, "Syntax error. 'OBJECTS' needed" );
    }
    if ( $value eq 'STATUS' ) {
        ( ( $token, $value ) = $self->get_token() ) || return;
        if ( $value =~ m/^(current|deprecated|obsolete)$/o ) {
...
sub parse_notificationgroup {
    ...
    if ( $value eq 'NOTIFICATIONS' ) {
        ...
        return $self->assert( MIBERROR, $self->{'filename'},
            $self->{'lineno'}, "Syntax error. 'NOTIFICATIONS' needed" );
    }
    if ( $value eq 'STATUS' ) {
        ( ( $token, $value ) = $self->get_token() ) || return;
        if ( $value =~ m/^(current|deprecated|obsolete)$/o ) {
...
sub parse_agentcapabilities {
    ...
    if ( $value eq 'PRODUCT-RELEASE' ) {
        ...
        return $self->assert( MIBERROR, $self->{'filename'},
            $self->{'lineno'}, "Syntax error. 'PRODUCT-RELEASE' needed" );
    }
    ...
    while ( defined $token && $token != $ASSIGNMENT ) {
            ...
            && $token != $ASSIGNMENT )
        {
            ...
            while ( $value eq 'VARIATION' ) {
                ...
                if ( $value eq 'SYNTAX' ) {
                    my $type = $self->parse_type();
                    $$data{'supports'}{$name}{'variation'}{$val}{'syntax'} =
                        $type;
                    ( ( $token, $value ) = $self->get_token() ) || return;
                }
                if ( $value eq 'WRITE-SYNTAX' ) {
                    my $type = $self->parse_type();
                    $$data{'supports'}{$name}{'variation'}{$val}
                        {'write-syntax'} = $type;
                    ( ( $token, $value ) = $self->get_token() ) || return;
                }
                if ( $value eq 'ACCESS' ) {
                ...
                if ( $value eq 'DEFVAL' ) {
                    ...
                    ( ( $token, $value ) = $self->get_token('IDENTIFIER') )
                        || return;
                    $$data{'supports'}{$name}{'variation'}{$val}{'defval'} =
                        $value;
                    ( ( $token, $value ) = $self->get_token('}') ) || return;
                    ( ( $token, $value ) = $self->get_token() )    || return;
                }
...
sub parse_traptype {
    ...
    if ( $value eq 'ENTERPRISE' ) {
        ...
        return $self->assert( MIBERROR, $self->{'filename'},
            $self->{'lineno'}, "Syntax error. 'ENTERPRISE' needed" );
    }
    ...
    if ( $token == $ASSIGNMENT ) {
        ...
        return $self->assert( MIBERROR, $self->{'filename'},
            $self->{'lineno'}, "Should be '::='instead of '$value'" );
    }
    $data;
}
...
sub parse_objecttype {
    ...
    if ( $value eq 'ACCESS' || $value eq 'MAX-ACCESS' ) {
        if ( $value eq 'MAX-ACCESS' ) {
                ...
                )
            {
                ...
                return $self->assert( MIBERROR, $self->{'filename'},
                    $self->{'lineno'}, "Unknown acces type ($value)" );
            }
        } else {    # 'ACCESS'
            return $self->assert(
                MIBERROR,          $self->{'filename'},
            ...
            ( ( $token, $value ) = $self->get_token() ) || return;
            if ( $value
                =~ m/^(read-(only|write)|write-only|not-accessible)$/o )
            {
        ...
        if ( $self->{'accept_smiv1'} && !$self->{'accept_smiv2'} ) {
            return $self->assert( MIBERROR, $self->{'filename'},
                $self->{'lineno'}, "Syntax error. 'ACCESS' needed" );
        } elsif ( !$self->{'accept_smiv1'} && $self->{'accept_smiv2'} ) {
            return $self->assert( MIBERROR, $self->{'filename'},
                $self->{'lineno'}, "Syntax error. 'MAX-ACCESS' needed" );
        } else {
            return $self->assert( MIBERROR, $self->{'filename'},
                $self->{'lineno'},
    ...
    if ( $value eq 'STATUS' ) {
            ...
            && $value =~ m/^(current|obsolete|deprecated)$/o )
        {
            ...
            return $self->assert( MIBERROR, $self->{'filename'},
                $self->{'lineno'}, "Unknown status ($value)" );
        }
        ( ( $token, $value ) = $self->get_token() ) || return;
    } else {
        return $self->assert( MIBERROR, $self->{'filename'},
    ...
    if ( $value eq 'AUGMENTS' && $self->{'accept_smiv2'} ) {
        return $self->assert( MIBERROR, $self->{'filename'},
            $self->{'lineno'}, "Can't define both 'INDEX' and 'AUGMENTS'" )
...
sub parse_imports {
    ...
    while ( defined $token && $value ne ';' ) {
        ...
        {
            return $self->assert( MIBERROR, $self->{'filename'},
                $self->{'lineno'}, "two values must be separated by a comma" )
                if $elem;
            push @$list, $value;
            $elem = 1;
        } elsif ( $value eq ',' ) {
            return $self->assert( MIBERROR, $self->{'filename'},
                $self->{'lineno'}, "value expected. ',' found" )
                unless $elem;
            $elem = 0;
        } elsif ( $token == $FROM ) {
            $elem = 0;
...
sub import_modules {
    ...
    for my $k ( keys %{ $self->{'imports'} } ) {
        ...
        for my $item ( @{ $self->{'imports'}{$k} } ) {
            warn
                "DEBUG: importing symbol $item from $k for $self->{'name'}...\n"
                if $self->{'debug_lexer'};
            if ( defined $mib->{'nodes'}{$item} ) {

                # resolve OID to break the dependencies
                my @a = split /\./,
                    $mib->convert_oid( $mib->resolve_oid($item) );
                ...
                for my $macro ( @{ $mib->{'macros'} } ) {
                    $found++, push( @{ $self->{'macros'} }, $item )
                        if $macro eq $item;
                }
                $self->assert( MIBWARN, $self->{'filename'},
                    $self->{'lineno'}, "can't find '$item' in $k" )
                    unless $found;
            }
        }
